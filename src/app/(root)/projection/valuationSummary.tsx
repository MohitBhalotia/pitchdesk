...
export default function ValuationSummary({ result, stage }: { result: any; stage: string }) {
    ...
    const valuationMin =
        valuationCandidates.length > 0 ? Math.min(...valuationCandidates) : null;
    const valuationMax =
        valuationCandidates.length > 0 ? Math.max(...valuationCandidates) : null;

    const annualRevenue = p?.annual_revenue ?? null;

    const monthly: ProjectionMonth[] = Array.isArray(p?.monthly)
        ? p.monthly
        : [];
